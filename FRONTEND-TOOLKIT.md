...
| `impeccable` | pbakaus/impeccable | 59 automatic detector rules + LLM critique that catch the tells of generic AI-generated UI (Arial/Inter overuse, cards nested in cards, low-contrast gray-on-color text, dated bounce/elastic animation). Ships a shared vocabulary of commands: `polish`, `critique`, `audit`, `animate`, `harden`, `distill`. |
